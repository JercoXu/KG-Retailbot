...
# This is a general class that uses the aip module which allows to update the mdp structure given an observation and a preference.
...
class AiAgent(object):
    def __init__(self, mdp):
        self._mdp = copy.deepcopy(mdp)  # This contains the mdp structure for the active inference agent
        ...
        self.policy_post_u = np.zeros(
            [self.n_policies, self.t_horizon])  # Initialize vector to contain posterior probabilities of actions
        ...
        for this_policy in range(self.n_policies):  # Loop over the available policies
            ...
            for tau in range(self.t_horizon):  # Loop over future time points
                ...
                if tau <= self.t_horizon:
                    lnA = np.reshape(self.aip_log(np.dot(self.likelihood_A, self.sparse_O[:, tau])),
                                     (self.n_states, 1))  # lnA.o_tau
                else:
                ...
                if tau == 0:
                    ...
                    lnB_past = self.aip_log(
                        np.dot(self.fwd_trans_B[:, :, self.policy_indexes_v[this_policy]], s_tau_past))
                ...
                if tau >= self.t_horizon - 1:
                    ...
                    lnB_future = self.aip_log(
                        np.dot(self.bwd_trans_B[:, :, self.policy_indexes_v[this_policy]], s_tau_future))
                ...
                # Compute F
                self.F[this_policy] = self.F[this_policy] + np.dot(self.post_x[:, tau, this_policy],
                                                                   self.aip_log(s_pi_tau) - lnB_past - lnA)
    ...
    def infer_policies(self):
        ...
        for this_policy in range(self.n_policies):
            for future_time in range(1, self.t_horizon):
                ...
                o_pi_tau = np.argmax(np.dot(self.fwd_trans_B[:, :, self.policy_indexes_v[this_policy]],
                                            self.post_x[:, future_time - 1, this_policy]))
                self.sparse_O[o_pi_tau, future_time] = 1
                self.G[this_policy] = self.G[this_policy] + np.dot(
                    self.aip_log(self.sparse_O[:, future_time]) - np.transpose(self._mdp.C),
                    self.sparse_O[:, future_time]) + np.dot(
                    np.diagonal(np.dot(np.transpose(self.likelihood_A), self.aip_log(self.likelihood_A))),
                    np.reshape(self.post_x[:, future_time, this_policy],
                               (len(self.post_x[:, future_time, this_policy]), 1)))
        ...
        self._mdp.D = self.aip_norm(self._mdp.D + self._mdp.kappa_d * self.post_x_bma[:, 0].reshape(self.n_states,
                                                                                                    1))  # Take first policy (idle) at current time, so simple state update, and update prior D
