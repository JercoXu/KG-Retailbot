...
class Neo4jKnowledgeBase(KnowledgeBase):
    def __init__(self, uri, user, password):
        ...
        self.relation_attributes = {
            "Object": {"PoseMeasure": "pose", "PositionMeasure": "positionedAt"},
            "Data": {"PoseMeasure": "pose", "PositionMeasure": "positionedAt"},
            "Agent Plan": {"Plan Action": "contains"},
            "Task": {"Subtask": "abstractPart"},
            "PhysicalQuantity": {"PoseMeasure": "pose"},
            "PositionMeasure": {},
            "PoseMeasure": {},
            "Subtask": {},
        }
    ...
    def do_get_attributes_of_object(self, object_type) -> List[Text]:
        # with self._driver.session(database='kgdata') as session:
    ...
    async def get_objects(
            self, object_type: Text, attributes: List[Dict[Text, Text]], limit: int = 10
    ) -> List[Dict[Text, Any]]:
        """
...
if __name__ == "__main__":
    ...
    kb = Neo4jKnowledgeBase("bolt://localhost:11003", "neo4j", "xxx")
    loop = asyncio.get_event_loop()
    #
    result = loop.run_until_complete(kb.get_objects("Object", [], 5))
    # print(result)

    # result = loop.run_until_complete(kb.get_object("Object", "milk"))
    # print(result)
    # result = loop.run_until_complete(
    #     kb.get_objects("Data", [{"name": "PoseMeasure", "value": "base_loc"}], 4)
    # )
    # print(result)
    #
    # result = loop.run_until_complete(kb.get_attributes_of_object("Object"))
    # print(result)
